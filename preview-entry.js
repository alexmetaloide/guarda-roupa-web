...
function makePart(p){if(p.shape==='rod'){const geo=new THREE.CylinderGeometry(p.diameter/2,p.diameter/2,p.length,24);const mesh=new THREE.Mesh(geo,new THREE.MeshStandardMaterial({color:palette.metal,metalness:.7,roughness:.25}));mesh.rotation.z=Math.PI/2;mesh.position.set(p.x+p.length/2,p.y,p.z);return mesh;}const geo=new THREE.BoxGeometry(p.w,p.h,p.depth);const mat=new THREE.MeshStandardMaterial({color:colorFor(p),roughness:.58,transparent:p.material==='back',opacity:1});const mesh=new THREE.Mesh(geo,mat);mesh.position.set(p.x+p.w/2,p.y+p.h/2,p.z+p.depth/2);mesh.castShadow=true;mesh.receiveShadow=true;return mesh;}
...
function setup3D(){const host=$('scene3d');host.append(renderer.domElement);setupPointer(host);window.addEventListener('resize',resize);resize();const get=()=>window.getConfiguration?.();window.refresh3D=()=>{if(get)renderModel(get());};if(get)renderModel(get());document.querySelectorAll('[data-key]').forEach(el=>{el.addEventListener('change',()=>window.refresh3D());el.addEventListener('input',()=>window.refresh3D());});document.getElementById('front3d').onclick=()=>{controls.theta=0;controls.phi=1.45;controls.radius=620;};document.getElementById('angle3d').onclick=()=>{controls.theta=.68;controls.phi=1.05;controls.radius=620;};document.getElementById('zoom-in3d').onclick=()=>controls.radius=Math.max(110,controls.radius*.85);document.getElementById('zoom-out3d').onclick=()=>controls.radius=Math.min(1200,controls.radius*1.18);document.getElementById('hide-doors3d').onchange=e=>setDoors(e.target.checked);document.getElementById('ghost3d').onchange=e=>{const ghost=e.target.checked;root.children.forEach(m=>{if(m.material?.transparent!==undefined&&m.userData.p?.material!=='door')m.material.opacity=ghost?.2:1;});};document.getElementById('open-drawers3d').onclick=()=>state.actions.forEach(a=>{if(a.kind!=='porta')setOpen(a.id,true);});document.getElementById('close-all3d').onclick=()=>state.actions.forEach(a=>setOpen(a.id,false));$('expand3d').onclick=()=>{const dedicated=/preview3d\.html$/i.test(window.location.pathname);if(dedicated){const mainPage=window.location.protocol==='file:'?'Editor_Guarda_Roupa.html':'index.html';window.close();setTimeout(()=>{if(!window.closed)window.location.assign(mainPage);},120);return;}const cfg=get?.();if(cfg)localStorage.setItem('guarda-roupa-preview-config',JSON.stringify(cfg));const tab=window.open('preview3d.html','_blank','noopener');if(!tab)window.location.assign('preview3d.html');};renderer.domElement.addEventListener('dblclick',()=>{});controls.update();frame();}
window.setup3D=setup3D;
;const __previewDefaults={"esp_fundo":0.6,"opcoes_corredica":["Telescópica - folga total 27 mm","Oculta - folga total 10 mm"],"largura":200,"altura":265,"profundidade":60,"esp_estrutura":1.5,"esp_frente":1.5,"esp_prateleira":1.5,"altura_interna_inferior":195,"diferenca_profundidade_laterais":7,"altura_base_integrada_guarda":5,"quantidade_gavetas_esquerda":0,"quantidade_gavetas_direita":0,"quantidade_sapateiras_esquerda":0,"quantidade_sapateiras_direita":0,"altura_frente_sapateira":7,"esp_fundo_sapateira":0.6,"espacamento_sapateira":8,"altura_gaveteiro_esquerda":90,"altura_gaveteiro_direita":90,"engrosso_gaveteiro_esquerda_lateral_esquerda":"Não","engrosso_gaveteiro_esquerda_lateral_direita":"Não","engrosso_gaveteiro_direita_lateral_esquerda":"Não","engrosso_gaveteiro_direita_lateral_direita":"Não","esp_engrosso_gaveteiro_esquerda":1.5,"esp_engrosso_gaveteiro_direita":1.5,"esp_tamponamento_gaveteiro_esquerda":1.5,"esp_tamponamento_gaveteiro_direita":1.5,"largura_gaveteiro_esquerda":0,"largura_gaveteiro_direita":0,"alturas_frentes_esquerda":[],"alturas_frentes_direita":[],"alturas_frentes_manuais_esquerda":[],"alturas_frentes_manuais_direita":[],"recuo_gavetas_esquerda":0.4,"recuo_gavetas_direita":0.4,"recuo_gavetas_superior":0,"recuo_gavetas_inferior":1,"folga_entre_frentes_gavetas":0.4,"tipo_corredica_gavetas":"Telescópica - folga total 27 mm","quantidade_prateleiras_esquerda":0,"quantidade_prateleiras_direita":0,"criar_colmeia_esquerda":"Não","criar_colmeia_direita":"Não","vaos_colmeia_esquerda":2,"vaos_colmeia_direita":2,"alturas_prateleiras_esquerda":[],"alturas_prateleiras_direita":[],"modo_cabideiro_esquerda":"Nenhum","modo_cabideiro_direita":"Nenhum","recuo_varao_superior_esquerda":9.5,"recuo_varao_superior_direita":9.5,"altura_varao_inferior_esquerda":90,"altura_varao_inferior_direita":90,"diametro_cabideiro":2.5,"criar_portas_correr":"Sim","sobreposicao_portas_correr":3,"folga_portas_correr":0.4};window.getConfiguration=()=>{try{return {...__previewDefaults,...JSON.parse(localStorage.getItem("guarda-roupa-preview-config")||"{}")}}catch{return __previewDefaults}};setup3D();}catch(__e){console.error(__e);const __s=document.getElementById("scene3d-status");if(__s)__s.textContent="Não foi possível carregar a prévia 3D: "+(__e?.message||__e);}